...
public class SoundInstanceEditorUnitySound : MonoBehaviour
{
    ...
    private List<SoundInstanceEditorProperty> audioSourceProperties;
    ...
    private void UpdatePropertyCurves(){
        ...
        {
            SoundInstanceEditorProperty property = audioSourceProperties[i];
            ...
            // retrieve input value either through external script or through slider value
            // float inputValue = reflectionScriptProperty != null ? Convert.ToSingle(reflectionScriptProperty.GetValue(script)) : property.inputValue;

            // set both fields
            // property.inputValue = inputValue;
            // property.outputValue = outputValue;
            ...
            if(reflectionAudioSourceProperty != null && audioSource != null){
                if(reflectionAudioSourceProperty.PropertyType == typeof(bool))
                {
                    ...
                    reflectionAudioSourceProperty.SetValue(audioSource, property.outputValue);
                }
            }
        }
    }
    ...
    private void AddAudioSourcePropertyFromDropdown() {
        ...
        SoundInstanceEditorProperty newAudioSourceProperty = new SoundInstanceEditorProperty();
        ...
        if(audioSourceProperties == null) { audioSourceProperties = new List<SoundInstanceEditorProperty>(); }
    ...
    private void LoadAudioSourcePropertiesFromPreset() {
        ...
        if(audioSourcePropertiesPresets.Length == 0 || selectedPropertyPresetIndex == 0) {
            ...
            audioSourceProperties = new List<SoundInstanceEditorProperty>();
            ...
            SoundInstanceEditorProperty[] properties = audioSourcePropertiesPresets[selectedPropertyPresetIndex - 1].propertiesList;
    ...
    public void DrawInspectorGUI() {
        ...
        if(editorLevelProperty != null) { 
            editorLevelString += " (controlled by script)"; 
            editorLevel = Convert.ToSingle(editorLevelProperty.GetValue(script));
        }
        EditorGUI.BeginDisabledGroup(editorLevelActive == false || editorLevelProperty != null);
        ...
        if(audioSourceProperties != null)
        {
            ...
            for (int i = 0; i < audioSourceProperties.Count; i++)
            {
                ...
                if(audioSourcePropertyFoldouts[i]){
                    ...
                    // disables the input value, if the value is controlled by script
                    EditorGUI.BeginDisabledGroup(reflectionScriptProperty != null || editorLevelActive);
...
[CustomEditor(typeof(SoundInstanceEditorUnitySound))]
public class SoundInstanceEditorUnitySoundInspector : UnityEditor.Editor
{
    SoundInstanceEditorUnitySound soundInstanceEditor;

    private void OnEnable()
    {
        soundInstanceEditor = (SoundInstanceEditorUnitySound)target;
        soundInstanceEditor.LoadAudioSourcePropertiesResources();
        // soundInstanceEditor.InitializeAudioSourceProperties();
    }
